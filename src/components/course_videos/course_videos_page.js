...
const CourseVideosPage = () => {
  ...
  const fetchDocuments = useCallback(async (videoId, chapterId) => {
    try {
      const response = await axios.get(config.API_URL + '/documents', {
        params: {
          courseId,
          chapterId,
          videoId
  ...
  const fetchData = useCallback(async () => {
    try {
      const [chaptersResponse, videosResponse, quizzesResponse] = await Promise.all([
        axios.get(config.API_URL + '/courses/' + courseId + '/chapters'),
        axios.get(config.API_URL + '/courses/' + courseId + '/videos'),
        axios.get(config.API_URL + '/courses/' + courseId + '/quizzes')
      ]);
  ...
  const handleDownload = async (document) => {
    try {
      window.open(config.API_URL + '/documents/' + document.id + '/download', '_blank');
    } catch (error) {
