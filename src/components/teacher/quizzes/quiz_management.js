...
const QuizManagement = () => {
  ...
  const fetchCourses = async () => {
    try {
      ...
      const response = await axios.get(`${config.apiUrl}/teacher/courses`, {
  ...
  const fetchQuizzes = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.get(`${config.apiUrl}/teacher/quizzes`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleDelete = async (quizId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${config.apiUrl}/teacher/quizzes/${quizId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
