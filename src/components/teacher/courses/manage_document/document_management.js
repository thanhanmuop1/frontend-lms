...
const DocumentManagement = ({ visible, onCancel, courseId, chapterId, videoId }) => {
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(config.API_URL + '/teacher/courses/' + courseId + '/documents', {
        headers: {
  ...
  const handleDelete = async (documentId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(config.API_URL + '/documents/' + documentId, {
  ...
  const handleDownload = async (document) => {
    try {
      const token = localStorage.getItem('token');
      window.open(config.API_URL + '/teacher/documents/' + document.id + '/download', '_blank');
    } catch (error) {
  ...
  const uploadProps = {
    name: 'file',
    action: config.API_URL + '/teacher/courses/' + courseId + '/documents',
    headers: {
