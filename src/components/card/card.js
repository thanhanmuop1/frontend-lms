...
const CardComponent = () => {
    ...
    const fetchCourses = async () => {
      try {
        if (userRole === 'teacher' && token) {
          const myCoursesResponse = await axios.get(config.API_URL + '/teacher/courses', {
            headers: { Authorization: `Bearer ${token}` }
          });
          setMyCourses(myCoursesResponse.data);
        } else {
          const allCoursesResponse = await axios.get(config.API_URL + '/courses');
  ...
  const checkEnrollmentStatus = async (courseId) => {
    try {
      const response = await axios.get(config.API_URL + '/courseEnroll/check/' + courseId, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleEnroll = async (courseId) => {
    try {
      await axios.post(`${config.apiUrl}/courseEnroll/enroll`, { courseId }, {
        headers: { Authorization: `Bearer ${token}` }
      });
